...
const ProductPage = () => {
   const { data, variantId } = useLoaderData();
  ...
  useEffect(() => {
    if (selectedVariant) {
      setMedias(
        selectedVariant.variantMedias.length > 0
          ? selectedVariant.variantMedias
    ...
    <div className="w-full flex flex-col items-center justify-center font-body">
      <div className="w-full flex h-auto items-start justify-center relative">
        ...
        <div className="w-1/2 min-h-screen  sticky top-0 right-0 flex items-center justify-center  ">
            <ProductDetails product={product} selectedVariant={selectedVariant} onSelectVariant={setSelectedVariant} />
        </div>
      </div>
    </div>
  );
